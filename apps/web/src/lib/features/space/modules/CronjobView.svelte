...
const cronjobCanEditSchedule = $derived(cronjob.canEditSchedule);
							...
							<section class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_260px]">
								<div class="min-w-0 space-y-5">
									...
									<div class="space-y-2">
										...
										<textarea id="cronjob-edit-system-instructions" bind:value={cronjob.formSystemInstructions} rows="4" maxlength={MAX_PROMPT_SYSTEM_INSTRUCTIONS_LENGTH} placeholder={cronjobDetail.hasSystemInstructions ? 'Replace configured instructions' : 'Optional'} disabled={cronjob.formClearSystemInstructions || !cronjobCanEditSchedule} class="w-full resize-y rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[13px] leading-6 text-text-primary transition-colors placeholder:text-text-placeholder focus:border-brand/50 focus:outline-none disabled:opacity-45"></textarea>
										...
										{#if cronjobDetail.hasSystemInstructions}
											<label class="inline-flex min-h-8 items-center gap-2 text-[12px] text-text-secondary"><input type="checkbox" bind:checked={cronjob.formClearSystemInstructions} disabled={!cronjobCanEditSchedule} class="h-4 w-4 accent-brand disabled:opacity-45" /><span>Clear configured instructions</span></label>
										{/if}
									</div>
								</div>
								<aside class="space-y-5 text-[13px]">
									<div class="space-y-1.5"><label class="block text-[10px] font-medium uppercase tracking-wider text-text-placeholder" for="cronjob-edit-expression">Schedule</label><input id="cronjob-edit-expression" type="text" bind:value={cronjob.formExpression} disabled={!cronjobCanEditSchedule} class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 font-mono text-[13px] text-text-primary transition-colors focus:border-brand/50 focus:outline-none disabled:opacity-45" /></div>
									<div class="space-y-1.5"><label class="block text-[10px] font-medium uppercase tracking-wider text-text-placeholder" for="cronjob-edit-timezone">Timezone</label><input id="cronjob-edit-timezone" type="text" bind:value={cronjob.formTimezone} disabled={!cronjobCanEditSchedule} class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[13px] text-text-primary transition-colors focus:border-brand/50 focus:outline-none disabled:opacity-45" /></div>
									<div class="space-y-2"><div class="text-[10px] font-medium uppercase tracking-wider text-text-placeholder">Model</div><button type="button" class="flex min-h-10 w-full items-center justify-between gap-3 rounded-[6px] border border-border-subtle bg-bg-elevated/35 px-3 py-2 text-left transition-colors hover:bg-bg-hover disabled:opacity-45" disabled={!cronjobCanEditSchedule} onclick={() => cronjob.openModelSelector('edit')}><span class="min-w-0 truncate text-[13px] text-text-primary">{cronjobModelLabel(cronjob.formModel)}</span><Settings class="h-3.5 w-3.5 shrink-0 text-text-placeholder" /></button>{#if cronjob.formModel}<button type="button" class="text-[11px] text-text-placeholder transition-colors hover:text-text-secondary disabled:opacity-45" disabled={!cronjobCanEditSchedule} onclick={() => { cronjob.formModel = null; }}>Use default model</button>{/if}</div>
								</aside>
							</section>
