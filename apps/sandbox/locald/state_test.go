...
func TestPermitRuntimeIdentityPersistsAndFencesClaim(t *testing.T) {
	store, err := OpenState(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := store.PutPermit(attemptID, spaceID, replicaID, runtimeID, baseID, 1, expiresAt, "local_agent", attemptID); err != nil {
		t.Fatal(err)
	}
	permit, err := store.PermitContext(attemptID)
	if err != nil || permit == nil {
		t.Fatalf("read persisted permit: %v %#v", err, permit)
	}
	if permit.RuntimeID != runtimeID {
		t.Fatalf("runtime identity was not persisted: %#v", permit)
	}
	if err := store.ClaimLocalRuntimePermit(attemptID, spaceID, replicaID, otherID, baseID, 1, expiresAt); err == nil {
		t.Fatal("claim from another runtime was accepted")
	}
