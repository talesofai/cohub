...
import { buildVolcCorpusContext, normalizeAsrSessionOptions } from "../src/asr/options.js";
...
const decodeFullClientRequest = (frame: Buffer) => {
  ...
  return JSON.parse(gunzipSync(payload).toString("utf-8")) as Record<string, unknown>;
};
...
test("buildVolcCorpusContext serializes hotwords and newest context", () => {
  ...
  assert.equal(context, JSON.stringify({
    hotwords: [{ word: "Cohub" }],
...
test("encodeFullClientRequest maps ASR tuning into the Volc request payload", () => {
  const payload = decodeFullClientRequest(encodeFullClientRequest({
    uid: "user-1",
    endWindowSizeMs: 600,
  ...
  assert.equal(corpus.context, JSON.stringify({ hotwords: [{ word: "Cohub" }] }));
});
