...
export const spaceLocalAgentPolicies = v2.table(
  "space_local_agent_policies",
  {
    ...
    workspaceMode: varchar("workspace_mode", { length: 30 }).$type<WorkspaceSyncMode>().notNull().default("two_way_safe"),
    updatedBy: varchar("updated_by", { length: 255 }).notNull(),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
...
export const workspaceReplicas = v2.table(
  "workspace_replicas",
  {
    ...
    initialChoice: varchar("initial_choice", { length: 20 }).$type<"use-cloud" | "use-local" | "merge" | null>(),
...
export const workspaceSyncCycles = v2.table(
  ...
  (table) => ({
    activeSpaceIdx: uniqueIndex("v2_uq_workspace_sync_cycles_active_space").on(table.spaceId).where(sql`${table.status} in ('planned', 'transferring', 'applying_cloud')`),
