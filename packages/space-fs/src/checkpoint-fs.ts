...
import { getMimeType, isCheckpointTextMime as isTextMime } from "./mime.js";
...
export function checkpointFsJsonError(error: unknown) {
  ...
  return { status: 500, body: { code: "checkpoint_fs_error", message: "Checkpoint file operation failed." } };
}
...
export function createCheckpointFs(deps: SpaceFsDeps) {
  const { config, db, redis, serviceName } = deps;
  ...
  async function getCheckpointForSpace(spaceId: string, checkpointId: string, observation?: CheckpointFsObservation) {
  ...
  async function getCachedBlob(repoDir: string, checkpoint: CheckpointRecord, path: string, observation?: CheckpointFsObservation, purpose = "blob") {
    ...
    const readCache = async () => {
      ...
      const blob = await redis.get(key).then(decodeCachedBlob).catch(() => {
        redisError = true;
        return null;
      });
    ...
    if (observation) {
      await observeStage(
        ...
        async () => {
          let written = true;
          await redis.set(key, encodeCachedBlob(result.stdout), "EX", CACHE_TTL_SECONDS).catch(() => {
            written = false;
          });
          return written;
        },
      ...
      await redis.set(key, encodeCachedBlob(result.stdout), "EX", CACHE_TTL_SECONDS).catch(() => undefined);
    }
  ...
  async function getAssetForPath(repoDir: string, checkpoint: CheckpointRecord, path: string, observation?: CheckpointFsObservation) {
    ...
    const readCache = async () => {
      let redisError = false;
      const value = await redis.get(key).catch(() => {
        redisError = true;
        return null;
      });
    ...
    if (observation) {
      await observeStage(
        ...
        async () => {
          let written = true;
          await redis.set(key, JSON.stringify(asset), "EX", CACHE_TTL_SECONDS).catch(() => {
            written = false;
          });
          return written;
        },
      ...
      await redis.set(key, JSON.stringify(asset), "EX", CACHE_TTL_SECONDS).catch(() => undefined);
    }
    return asset;
  }
  ...
  async function listCheckpointDirectory(input: { spaceId: string; checkpointId: string; path?: string }): Promise<SpaceFsTreeResponse> {
    return await observeCheckpointFs("tree", input, async (observation) => {
      ...
      await observeStage(
        ...
        async () => {
          let written = true;
          await redis.set(key, JSON.stringify(response), "EX", CACHE_TTL_SECONDS).catch(() => {
            written = false;
          });
          return written;
        },
  ...
  async function readCheckpointFile(input: { spaceId: string; checkpointId: string; path?: string }): Promise<SpaceFsFileResponse> {
  ...
  return {
    CheckpointFsError,
    listCheckpointDirectory,
    readCheckpointFile,
    getCheckpointForSpace,
    checkpointFsJsonError,
  };
}

export type CheckpointFs = ReturnType<typeof createCheckpointFs>;
