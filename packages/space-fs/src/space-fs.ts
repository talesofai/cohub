...
import { getMimeType, isTextMime } from "./mime.js";
import { createSpaceGitignoreFilter, type SpaceFsVisibility } from "./ignore.js";
import type { SpaceFsCdn, FsCdnFileMeta } from "./cdn.js";
import { FS_CDN_READ_MANY_WAIT_TIMEOUT_MS, FS_CDN_READ_WAIT_TIMEOUT_MS } from "./cdn.js";
...
export { getMimeType, isTextMime };
export type { SpaceFsVisibility };
...
export function createSpaceFsOps(deps: SpaceFsDeps, cdn: SpaceFsCdn) {
  ...
  async function ensureSpaceWorkspaceReady(spaceId: string) {
  ...
  async function listSpaceDirectory(
    spaceId: string,
    path = "",
  ...
  async function readSpaceDirectoryFiles(
    spaceId: string,
    path: string,
    options?: { visibility?: SpaceFsVisibility },
  ...
  async function streamSpaceFile(
    spaceId: string,
    path: string,
    options?: { visibility?: SpaceFsVisibility },
  ...
  async function writeSpaceFile(spaceId: string, input: SpaceFsWriteFileInput) {
  ...
  async function createSpaceFileExclusive(spaceId: string, input: SpaceFsWriteFileInput) {
    const { target, relativePath } = await resolveTarget(spaceId, input.path);
    await mkdir(dirname(target), { recursive: true });
    const data = input.encoding === "base64" ? Buffer.from(input.content, "base64") : Buffer.from(input.content, "utf8");
  ...
  async function createSpaceDirectory(spaceId: string, path: string) {
  ...
  const deleteSpaceNode = async (spaceId: string, path: string, recursive = false) => {
    const { target, relativePath } = await resolveTarget(spaceId, path);
  ...
  async function moveSpaceNode(spaceId: string, input: SpaceFsMoveInput) {
...
export type SpaceFsOps = ReturnType<typeof createSpaceFsOps>;
